...
def extract_regulation_mentions(df, text_column='text', section_column='article_name'):
    """    
    Extrae SOLO menciones de reglamentos relacionados con la ley actual:
    ...
    Returns:
        DataFrame con las menciones de reglamentos extraídas
    """
    start_time = time.time()
    results = []
    
    print(f"Iniciando extracción de menciones de REGLAMENTOS a las {datetime.now().strftime('%H:%M:%S')}")
    
    # Patrones específicos de menciones de reglamentos (grupo SPECIFIC_LAW_MENTIONS)
    specific_law_mentions = [
        # Menciones a reglamentos de la ley
        (r'\b(?:esta\s+ley|esta\s+LEY)\s+y\s+sus\s+reglamentos\b', 'LEY_CON_REGLAMENTOS'),
        (r'\b(?:esta\s+ley|esta\s+LEY)\s+y\s+sus\s+Reglamentos\b', 'LEY_CON_REGLAMENTOS'),
        (r'\b(?:esta\s+ley|esta\s+LEY)\s+y\s+con\s+el\s+reglamento\b', 'LEY_CON_REGLAMENTOS'),
        (r'\b(?:esta\s+ley|esta\s+LEY)\s+y\s+con\s+el\s+Reglamento\b', 'LEY_CON_REGLAMENTOS'),
        (r'\b(?:Reglamento|REGLAMENTO)\s+de\s+la\s+presente\s+ley\b', 'LEY_Y_REGLAMENTOS'),
        (r'\b(?:Reglamento|REGLAMENTO)\s+de\s+esta\s+ley\b', 'LEY_Y_REGLAMENTOS'),
        (r'\b(?:Reglamento|REGLAMENTO)\s+de\s+dicha\s+ley\b', 'LEY_Y_REGLAMENTOS'),
    ]
    ...
    for idx, row in df.iterrows():
        ...
        # Extraer menciones de reglamentos
    ...
    print(f"Total de menciones de reglamentos extraídas: {total_mentions}")
    print("=" * 70)
